...
- **Érdemi rész** – a jelen fájl 1–7. fejezeteinek átdolgozott változata.
...
- A szöveg a **jelenlegi kódbázis** állapotához igazodik: külön backend- és frontendprojekt, saját kliensoldali router, központi API-réteg, auditnaplózás, CSV export, bemutató adatkészlet-generálás, fizetési csoport szinkron, lezárt verseny eredményjavítási feloldása, valamint mérkőzésindítási ütközésvédelmek.
...
Az **ütemezés** szintén sajátos kihívást jelent. Több kategória mérkőzéseit párhuzamosan, korlátozott számú pályán kell lejátszani. Egy-egy játékos egyszerre csak egy mérkőzésen vehet részt, és a mérkőzések között minimális pihenőidőre is szükség van. A szervező feladata ezeket a feltételeket manuálisan összehangolni, ami nagyobb versenyeken komoly adminisztrációs terhet jelent.

Végül a **helyszíni és operatív tényezők** is szerepet játszanak: késő megérkezés, visszalépés, sérülés, illetve utólagos korrekciós igények is előfordulhatnak valós versenyen. Egy versenykezelő rendszernek ezeket a helyzeteket is kezelhetővé kell tennie.

## 1.2 Csoportkörös és egyenes kieséses rendszerek

A tollaslabda versenyeken a leggyakrabban alkalmazott két lebonyolítási forma a **csoportkörös** (körmérkőzéses rendszer) és az **egyenes kieséses** (egyenes kieséses szakasz, ágrajz) rendszer, amelyeket sok versenyen kombinálva alkalmaznak.

A csoportkörös szakaszaszban a játékosokat kisebb csoportokba osztják, és minden játékos mindenki mással mérkőzésezik a csoporton belül. Az eredmény alapján tabella készül: a győzelmek száma, szett- és pontkülönbség alapján rangsort állítanak fel. A csoportok legjobbjait – jellemzően az első kettőt vagy négyest – továbbjuttatják a egyenes kieséses szakaszba.

Ha a mezőny nagy, a teljes körmérkőzés helyett **részleges körmérkőzéses rendszer** alkalmazható: ebben az esetben mindenki csak meghatározott számú mérkőzést játszik, nem pedig az összeset. Ez csökkenti a mérkőzésszámot, de egyúttal bonyolultabb párosítási logikát igényel, mivel gondoskodni kell arról, hogy a terhelés közel egyenletes maradjon.

Az egyenes kieséses szakaszban a továbbjutók egymás ellen mérkőznek, és a vesztes kiesik. A hagyományos ágrajzstruktúra elődöntőkből, döntőből és bronzmérkőzésből áll. A párosítások általában **kiemelésalapú** alapon történnek, hogy a legerősebb játékosok a lehető legkésőbb találkozzanak egymással.

A lebonyolítási forma megválasztása befolyásolja az adminisztrációt is: a csoportkörös szakasz sokkal több mérkőzést és pontosabb tabellaszámítást igényel, míg az egyenes kiesés egyszerűbb struktúrát, de szigorúbb progressziókezelést követel meg.

A holtversenyek kérdése mindkét esetben megjelenik. Ha a csoportkör végén több játékosnak azonos a pontszáma, a rendszernek egyértelmű szabályok szerint kell sorba rendezni őket. A tollaslabdában ehhez általában az egymás elleni eredményt, majd a szett- és pontkülönbséget veszik figyelembe. Ezek a **holtverseny-feloldási** szabályok bonyolult döntési logikát igényelnek, amelyet szoftveresen szükséges megvalósítani.
...
Ezek a megoldások több szempontból is korlátozottak. A **táblázatkezelők** – például Microsoft Excel vagy Google Sheets – jók adatok tárolására, de a párosítások automatikus generálása, a csoportállás kiszámítása és a mérkőzésállapotok kezelése sok manuális munkát igényel. Könnyen keletkeznek hibák, és az adatok konzisztenciájának megőrzése nehézkes.
...
Az **eredmények visszakereshetősége** is problémás lehet utólag: papíralapú nyilvántartásnál nehéz például megállapítani, hogy egy héttel korábbi döntő pontszáma mi volt, vagy hogy ki kapta a második helyezést egy adott kategóriában.
...
A rendszer fejlesztéséhez szükséges technológiai döntések a projekt céljaihoz és a megvalósítandó funkciókhoz igazodtak. Ez a fejezet bemutatja a webalapú alkalmazások általános felépítésének szempontjait, a MERN-stack mint választott technológiai megközelítés indokait, és az egyes komponensek szerepét a végső rendszerben.
...
Az **Express** egy minimalista, Node.js-alapú webalkalmazás-keretrendszer. Segítségével gyorsan felépíthetők REST-szemléletű API-végpontok, definiálható a köztesréteg-lánc, és jól strukturálható a backend kódbázis. Az Express szándékosan nem írja elő a teljes alkalmazásstruktúrát, így a fejlesztőnek lehetősége van saját logikai szétválasztást alkalmazni.
...
A **React** egy deklaratív, komponensalapú JavaScript könyvtár a felhasználói felületek felépítésére. A fejlesztőnek nem az egyes DOM-műveletek sorrendjét kell megadnia, hanem azt írja le, hogy az aktuális adatok alapján hogyan nézzen ki a felület – a React gondoskodik az optimális frissítésekről. A komponensalapú megközelítés elősegíti az újrafelhasználhatóságot és az egységes megjelenést.

A jelen projekthez a React-alkalmazás fejlesztői szervere és buildeszköze a **Vite**, amely gyors indítási időjével és hatékony gyors modulcsere-megoldásával javítja a fejlesztési élményt.
...
Az **Express** és a **Node.js** kombinációja jól bevált megközelítés REST API-k fejlesztéséhez. A köztesréteg-alapú architektúra egyszerűen kezelhetővé teszi a hitelesítés, a validáció és a naplózás egységesítését, a moduláris routerstruktúra pedig jól illeszkedik a versenykezelő rendszer erőforrás-alapú felosztásához.
...
- a sorsoláslezárásának lehetőségét,
...
A rendszernek támogatnia kell a csoportkörös lebonyolítást. Kisebb létszám esetén teljes körmérkőzéses rendszer, nagyobb mezőny esetén csonka körmérkőzéses rendszer is alkalmazható.

A rendszernek képesnek kell lennie:
- csoportok létrehozására,
- a csoportokhoz tartozó játékosok kezelésére,
- körmérkőzéses párosítás generálására,
...
A követelmény azért kiemelten fontos, mert a teljes további logika – állásszámítás, továbbjutás, egyenes kieséses – a mérkőzéseredményekre épül.
...
- konfigurálható holtverseny-feloldási szabályrendszer alkalmazását,
...
### FR-10 Továbbjutók kiválasztása és egyenes kieséses ág generálása
...
- a egyenes kieséses nyitókörének meghatározására,
- kiemelésalapú párosítások létrehozására,
- elődöntő, döntő és releváns esetben bronzmérkőzés generálására,
- csak egyenes kieséses kategória kezelésére.

A rendszer célja itt az, hogy az adminisztrátor minimális kézi munkával tudja elindítani az egyenes kieséses szakaszt.

### FR-11 Nyilvános kijelzőnézet és állapotvizualizáció
...
A nyilvános kijelzőnézet célja az operatív tájékoztatás, nem az adminisztráció kiváltása.
...
A rendszernek jól tagolt, bővíthető szerkezetben kell felépülnie. A backendben célszerű elválasztani az útvonalkezelő, szolgáltatási és modellrétegeket, a frontendben pedig az oldalak, komponensek és szolgáltatások felelősségi körét. Ez elősegíti az új funkciók hozzáadását és a hibajavítást.
...
- Nagyobb mezőny esetén a rendszer képes részleges körmérkőzéses rendszer logika alkalmazására.
...
- A már futó mérkőzések és az ütemezési adatok alapján nem engedhető meg pályaütközés vagy játékosütközés a mérkőzésindítás során.
...
- Holtverseny esetén a rendszer a konfigurált holtverseny-feloldási szabály logikát alkalmazza.
- A továbbjutók automatikusan meghatározhatók.
- A egyenes kieséses ág a továbbjutási adatokból felépíthető.

### AC-08 Egyenes kieséses szakasz kezelése

- A rendszer képes csak egyenes kieséses és csoportkör + egyenes kieséses szakasz kategória kezelésére.
- A egyenes kieséses szakaszban legalább elődöntő, döntő és releváns esetben bronzmérkőzés generálható.
- A egyenes kieséses mérkőzések a rendszerben nyomon követhetők és eredményezhetők.
...
- A rendszer bemutató adatokkal feltölthető.
...
- teljes elsődlegesen offline működés,
...
- **hitelesítési kontextus**, amely a bejelentkezett felhasználó állapotát kezeli.
...
- mérkőzésekhez,
- ütemezéshez,
- csoportállás nézethez,
- egyenes kieséses nézethez,
- nyilvános kijelzőnézethez,
...
- **útvonalkezelő réteg**, amely a HTTP végpontokat definiálja;
- **köztesréteg**, amely például hitelesítést végez;
- **szolgáltatási réteg**, amely az üzleti logikát valósítja meg;
- **modellréteg**, amely a MongoDB-ben tárolt entitások sémáit írja le.

Az útvonalkezelő réteg erőforrás-orientált szerkezetet követ. Külön végpontcsoportok tartoznak többek között az alábbi területekhez:
- hitelesítés,
- versenyek,
- kategóriák,
...
- auditnaplók,
- exportok,
- nyilvános kijelzőnézetek.

Az útvonalkezelő réteg felett fontos szerepet kap a hitelesítési köztesréteg. A védett végpontok csak hitelesített felhasználó számára érhetők el, míg a hitelesítési és bizonyos nyilvános kijelző jellegű végpontok ettől elkülönítve működnek. A jogosultsági modell alapelve, hogy minden versenyhez tartozik egy tulajdonos, és egy felhasználó kizárólag a saját versenyeihez tartozó adatokat érheti el.

A backend üzleti logikájának legfontosabb része a szolgáltatási rétegben található. Itt valósulnak meg többek között:
- a hitelesítéshez tartozó segédfüggvények,
- a konfigurációvalidáció,
- a badminton szabályrendszer szerinti eredményellenőrzés,
- a teljes és csonka körmérkőzéses rendszer párosításgenerálás,
- az ütemezési logika,
- az állásszámítás és holtverseny-kezelés,
- az egyenes kieséses ág generálása,
- a CSV export,
- az auditnaplózás,
- a tulajdonosi ellenőrzés.

Ez a szétválasztás azt eredményezi, hogy a útvonalkezelő fájlok elsősorban kérésfeldolgozási és validációs szerepet töltenek be, míg a nem triviális üzleti döntések külön szolgáltatási modulokba kerülnek.
...
- **nyilvános végpontok**, amelyek például a nyilvános kijelzőnézet megjelenítését szolgálják.

A hitelesített kommunikáció alapja Bearer token használata. A kliens a bejelentkezést követően eltárolja a tokent, majd ezt minden védett kérésnél elküldi az Authorization fejlécben. A backend köztesréteg szinten ellenőrzi ezt az információt, és csak sikeres hitelesítés esetén engedi tovább a kérést.

### 4.1.6 Tervezési döntések indoklása

A rendszer architektúrája tudatosan nem monolitikus nézetkódra vagy egyetlen nagy backend fájlra épül, mert a versenykezelési logika összetettsége ezt hosszabb távon nehezen karbantarthatóvá tenné. A körmérkőzéses rendszer generálás, a holtverseny-feloldási logika, az ütemezés és a egyenes kieséses felépítése önmagukban is különálló felelősségi területek, ezért indokolt ezek moduláris elkülönítése.

A frontend oldalon az adminisztrációs folyamatokat követő nézetstruktúra azért előnyös, mert a versenykezelés tipikusan nem egyszeri lineáris varázslóként zajlik. A szervező gyakran visszalép korábbi lépésekhez, módosít adatokat, majd újra megnyitja a mérkőzés- vagy csoportállás nézetet. Emiatt a nézetek közötti gyors navigáció és az aktuális versenykontextus megőrzése fontos tervezési követelmény volt.
...
A `Category` entitás egy versenyen belüli önálló versenyszámot vagy lebonyolítási egységet reprezentál. Egy kategória megadja azt a kontextust, amelyben a játékosok indulnak, a csoportok létrejönnek, a mérkőzések keletkeznek, és a csoportállás vagy egyenes kieséses logika lefut.

A kategória legfontosabb mezői:
- név,
- státusz,
- sorsolási verzió,
- sorsolás lezárásának ideje,
- check-in konfiguráció,
- csoportkörös mérkőzésszám paraméter,
- csoportcélméret,
- továbbjutók száma,
- egyenes kieséses méret,
- nemi besorolás,
- korosztály,
- lebonyolítási forma,
- holtverseny-feloldási szabályok,
- játék nélküli győzelem és incomplete policy.
...
- a számlázási nevet,
- a számlázási címet,
- opcionálisan egy fizetési csoport azonosítót.
...
- egy fizetési csoport több nevezéshez kapcsolódhat;
- egy nevezés legfeljebb egy fizetési csoporthoz tartozhat.

A megvalósítás fontos részlete, hogy a fizetési csoport nem csupán leíró vagy számlázási csoportosító szerepet tölt be, hanem a befizetettség konzisztenciáját is kezeli. A csoport szintű befizetés ezért a felületen és backend oldalon is tényleges tömeges adminisztratív műveletként jelenik meg.
...
- sorsolási verzió,
- eredménytípus,
- érvénytelenített státusz és kapcsolódó mezők,
- pályaszám,
...
- a egyenes kieséses mérkőzéseket,
- a normál lejátszott eredményeket,
- a speciális kimeneteleket,
- az ütemezési információkat.

A `groupId` mező opcionális, mert csak egyenes kieséses vagy csoporttól független mérkőzések esetén nincs szükség csoportkapcsolatra.

A `pairKey` és a hozzá kapcsolódó egyedi indexelési logika biztosítja, hogy ugyanazon sorsolási verzión belül ne keletkezhessenek duplikált párosítások. Ez különösen újrasorsolás vagy újragenerálási helyzetekben fontos.
...
Ez a modell rugalmasan használható különféle eseménytípusok naplózására, és különösen hasznos hibakeresési, auditálási és bemutatási célokra.
...
A játékosok, nevezések és fizetési csoport rekordok külön végpontcsoportokban kezelhetők.
...
A fizetési csoport műveletek főbb végpontjai:
...
A csoportkezelés a versenylogika egyik központi eleme, ezért külön végpontcsoportot kapott. A csoportok nem csupán tárolt objektumok, hanem a csoportállás, withdrawal és egyenes kieséses szakaszhoz kapcsolódó műveletek belépési pontjai is.
...
A csoportállás végpont a csoportkör befejezett mérkőzéseiből számolja ki az állást. A withsorsolás végpont külön üzleti jelentőséggel bír, mert képes a visszalépések különböző policy szerinti kezelésére. A csoportból generált egyenes kieséses szakasz végpontok pedig a továbbjutók kiválasztásán alapuló egyenes kieséses szakasz felépítését és továbbvitelét biztosítják.
...
A mérkőzések generálása csoportszinten történik. A státuszfrissítés és eredményrögzítés külön végpontokban valósul meg, mert ezek eltérő validációs logikát követnek. Az `outcome` végpont speciális kimenetelek, például játék nélküli győzelem vagy érvénytelenítés kezelésére szolgál. A mérkőzésindításnál a backend külön ellenőrzi a pálya- és játékosütközéseket, valamint azt is, hogy a mérkőzés rendelkezik-e ütemezési adatokkal.

Az ütemezés két szinten jelenik meg:
- csoportszintű ütemezésgenerálás,
- teljes versenyszintű globális ütemezésgenerálás.

Ez a kettős felosztás azért indokolt, mert más problémát old meg egyetlen csoport belső mérkőzéseinek kiosztása, mint több kategória közös pályahasználatának összehangolása.
...
### 4.3.9 Nyilvános kijelző végpont
...
- ütemezés,
- eredményrögzítés,
- csoportállás lekérdezés,
- egyenes kieséses ág generálása,
- egyenes kieséses továbbvitele.

Egy csak egyenes kieséses kategória esetén a folyamat rövidebb:
- kategória és játékosok létrehozása,
- sorsolás lezárása,
- kezdeti egyenes kieséses ág generálása,
- eredményrögzítés,
...
- nem lezárt csoportkörből nem generálható egyenes kieséses szakasz,
...
### 4.4.1 Csoportkör-generálás: teljes és csonka körmérkőzéses rendszer
...
- **teljes körmérkőzéses rendszer**, amikor minden játékos minden másik játékossal játszik;
- **csonka körmérkőzéses rendszer**, amikor minden játékos csak meghatározott számú mérkőzést játszik.

Kis létszámú csoport esetén a rendszer a teljes körmérkőzéses rendszer logikát alkalmazza. Ennek alapja a klasszikus körforgásos párosítási módszer, amely páros játékosszám esetén minden fordulóban teljes párosítást ad, páratlan játékosszám esetén pedig egy virtuális BYE helyet vezet be. A BYE párosításból nem keletkezik tényleges mérkőzés, így a generált lista csak valós párosításokat tartalmaz.

Nagyobb mezőny esetén a teljes körmérkőzés túl sok mérkőzést eredményezne, ezért a rendszer részleges körmérkőzéses rendszer logikát használ. Páros létszám esetén a teljes körmérkőzéses rendszer körforgásos fordulóiból csak az első `m` forduló kerül kiválasztásra, ahol `m` a játékosonként kívánt mérkőzésszám. Páratlan létszámnál ez a megoldás BYE torzuláshoz vezetne, ezért a rendszer egy külön, szabályos fokszámú gráfra épülő párosítási logikát alkalmaz. Ebben az esetben minden játékos pontosan azonos számú ellenfelet kap, és az `m` értéknek párosnak kell lennie.

A körmérkőzéses rendszer szolgáltatás legfontosabb céljai tehát:
...
A sorsoláslezárás logikája az alábbi fő lépésekből áll:

1. a rendszer összegyűjti az adott kategória ténylegesen induló, `main` jogosultságú játékosait;
2. csak egyenes kieséses kategória esetén ellenőrzi, hogy a játékosszám pontosan megfelel-e a megadott egyenes kieséses méretnek;
...
6. eltárolja a sorsolási verziót és a lezárás időpontját.

A csoportképzés jelenlegi megvalósítása egyszerű, soronként szétosztó elvet követ. Ez nem klasszikus adatkészlet-generálás, hanem olyan kiegyensúlyozó mechanizmus, amely a létrejövő csoportok létszámát igyekszik egyenletesen elosztani.

A check-inhez kapcsolódó grace idő lezárása külön logika. A határidő letelte után a meg nem jelent játékosok `friendly_only` státuszba kerülhetnek, és az őket érintő, még le nem játszott mérkőzések érvényteleníthetők. Ez a megoldás lehetővé teszi, hogy a rendszer alkalmazkodjon a valós versenyhelyzetekhez anélkül, hogy a teljes struktúrát újra kellene építeni.
...
Ez a logika azért lényeges, mert a csoportállás, a továbbjutás és a egyenes kieséses összes későbbi lépése a korrektül validált eredményekre épül.

### 4.4.4 Állásszámítás és holtverseny-feloldás

A csoportállás logika a rendszer egyik legfontosabb szakmai komponense. A cél nem pusztán a győzelmek összeszámlálása, hanem a holtversenyek szabályozott, reprodukálható feloldása.
...
Az elsődleges rangsorolás alapja a győzelmi arány, majd a győzelmek száma. Erre azért van szükség, mert speciális helyzetekben – például visszalépések vagy érvénytelenített mérkőzések miatt – a lejátszott mérkőzések száma eltérhet.
...
### 4.4.5 Továbbjutók kiválasztása és egyenes kieséses ág generálása

A egyenes kieséses ág generálása logikája az állásszámításra épül. A rendszer először meghatározza a továbbjutás alapját képező top N játékost. Ha a továbbjutási határon holtverseny áll fenn, és azt a holtverseny-feloldási logika sem oldja fel teljesen, a rendszer manuális feloldást vagy explicit felülbírálatot kérhet.

A továbbjutók kiválasztása után a rendszer a egyenes kieséses méretből meghatározza a kezdőkört:
...
A párosítások kiemelésalapú alapelv szerint jönnek létre. A jelenlegi implementációban ez azt jelenti, hogy a rendszer az elejéről és a végéről párosít:
- 1. helyezett – utolsó továbbjutó,
- 2. helyezett – utolsó előtti továbbjutó,
- és így tovább.

A csoportból generált egyenes kieséses és a csak egyenes kieséses kategória logikája azonos elvre épül, csak a bemenet eltér:
- csoportkör + egyenes kieséses szakasz esetén a csoportállás adja a bemenetet,
- csak egyenes kieséses esetén a kategória indulómezőnye közvetlenül szolgál alapul.

A következő egyenes kieséses kör generálása már a kész mérkőzések győzteseiből történik. Két-két győztes összevezetésével új mérkőzések jönnek létre a következő körben. Elődöntő után a rendszer automatikusan bronzmérkőzést is tud generálni a két vesztesből.

Ez a logika lehetővé teszi, hogy a egyenes kieséses ne egyszerre, teljes mélységében legyen előállítva, hanem fokozatosan, az előző kör tényleges eredményei alapján épüljön fel.

### 4.4.6 Visszalépés és speciális mérkőzéskimenetelek kezelése
...
A `delete_results` esetben az érintett játékos mérkőzései érvénytelenített státuszba kerülnek, és a csoportállás ezeket figyelmen kívül hagyja. Ez akkor indokolt, ha az adott eredmények torzítanák a teljes csoportot.

A `keep_results` esetben a már lejátszott mérkőzések eredménye megmarad, a még nem lejátszott mérkőzések pedig játék nélküli győzelem jellegű lezárást kaphatnak. A rendszer ezekhez `wo` típusú eredményt rendel.

A speciális mérkőzéskimenetelek külön végponttal kezelhetők, így a mérkőzéséletciklus nem csak normál lejátszott eredményekre épül.

A rendszer jelenlegi állapotában ehhez kapcsolódik a lezárt versenyek kontrollált eredményjavítási mechanizmusa is. A lezárás után a mérkőzések módosítása alapértelmezésben tiltott, de adminisztratív feloldással ideiglenesen engedélyezhető, majd a korrekció után újra visszazárható. Ez a megközelítés jobban illeszkedik a valós versenyhelyzetekhez, mint a korlátlan utólagos szerkeszthetőség.
...
A játékos rendelkezésre állási idejét a rendszer a legutóbbi mérkőzés vége és a minimális pihenőidő alapján számolja. A pálya elérhetősége a korábbi mérkőzés vége és a pályaforgatási idő alapján módosul.
...
Ez a logika egyszerűbb, mint egy PDF-generáló folyamat, ugyanakkor a versenyadatok archiválására és további feldolgozására már jelen formájában is alkalmas.
...
**Javasolt ábrahelyek:**  
*4.4. ábra – Körmérkőzéses és csonka körmérkőzéses rendszer párosítás logikája*  
*4.5. ábra – Csoportállás és holtverseny-feloldási szabály döntési folyamat*  
...
A backend Node.js környezetben futó Express alkalmazásként valósult meg. A belépési pontot az `index.js` fájl adja, míg az alkalmazás összeállítása a `src/app.js` fájlban történik. A rendszer ezen a ponton regisztrálja a köztesrétegeket, az útvonalakat és az adatbázis-kapcsolatot.
...
Ezt egészítik ki a segédjellegű szkriptfájlok, amelyek adatkészlet-generálási, önellenőrzési és automatizált végponttesztelési feladatokat látnak el.

### 5.1.1 Köztesréteg és alkalmazásindítás

Az alkalmazásindítás első lépése a környezeti változók betöltése és a MongoDB kapcsolat felépítése. A `src/app.js` fájl tartalmazza az Express példány létrehozását, a JSON és URL-encoded parser köztesrétegek bekapcsolását, valamint az útvonalcsoportok regisztrációját.

A backendben fontos szerepet játszik a hitelesítési köztesréteg (`src/middleware/auth.middleware.js`). Ennek feladata:
...
A megvalósítás egyik tudatos döntése az volt, hogy a védett végpontok előtt egy közös `requireAuth` köztesréteg kerül alkalmazásra. Ennek eredményeként az `/api/auth` és a `/public` kivételével minden `/api` útvonal csak hitelesített felhasználó számára érhető el. Ez egyszerűsíti a hozzáférésvédelmet, mert nem kell minden egyes útvonalkezelőben külön újra definiálni a hitelesítési ellenőrzést.
...
- a speciális mérkőzéskimeneteleket,
...
### 5.1.3 Útvonalkezelő réteg és végpontcsoportok

Az útvonalkezelő réteg erőforrásalapú szerkezetben készült. Az egyes végpontcsoportok külön fájlokba szervezve jelennek meg, így a rendszerben jól elkülönülnek az eltérő felelősségi körök.

A főbb útvonalkezelő fájlok:
- `auth.routes.js`
...
Az útvonalkezelő fájlok jellemzően az alábbi feladatokat látják el:
...
A nem triviális üzleti döntések tudatosan nem a útvonalkezelő fájlokban kaptak helyet, hanem külön szolgáltatási modulokban.

### 5.1.4 Szolgáltatási réteg és üzleti logika

A backend érdemi szakmai része a szolgáltatási rétegben valósult meg. A rendszer több különálló szolgáltatási fájlt használ, amelyek egymástól jól elkülönülő felelősségi kört fednek le.
...
- az egyenes kieséses ág generálását,
...
Az `ownership.service.js` külön figyelmet érdemel. A rendszer egyik fontos biztonsági és konzisztenciaelvét ez biztosítja, mivel segíti annak ellenőrzését, hogy egy adott verseny vagy kapcsolódó objektum valóban a bejelentkezett felhasználóhoz tartozik-e. Ez a megoldás csökkenti annak esélyét, hogy egy útvonalkezelő tévesen olyan erőforráshoz adjon hozzáférést, amely nem az aktuális felhasználó tulajdona.
...
### 5.1.5 Szkriptréteg: bemutató adatkészlet, önellenőrzés és füsttesztek

A backend egyik fontos gyakorlati erőssége, hogy nem kizárólag az alkalmazáskód készült el, hanem hozzá kapcsolódó szkriptkészlet is. A `src/scripts` mappában több olyan segédprogram található, amely a fejlesztést, a bemutatást és a validálást támogatja.

Ezek közül kiemelendő:
- a `seed_demo.js`,
- a `selftest.js`,
- az `e2e_*` és `smoke:*` jellegű szkriptfájlok.

A bemutató adatkészlet-generálás feladata egy ismert tesztfelhasználó és több, eltérő állapotú verseny létrehozása. Ez különösen hasznos a rendszer bemutatásakor, mert nem szükséges minden alkalommal manuálisan felépíteni a teljes bemutató állapotot.

Az önellenőrző és füstteszt jellegű scriptek többek között az alábbi területeket fedik le:
- hitelesítés és ownership,
- check-in és sorsoláslezárás,
- csoportállás és holtverseny-feloldási szabály,
- egyenes kieséses működés,
- globális ütemezés,
- eredményvalidáció,
- auditnaplózás,
- CSV export,
- nyilvános kijelzőnézet,
- nevezési díj logika.

Ez a szkriptkészlet a megvalósítás szempontjából azért fontos, mert a rendszer nem kizárólag manuális kipróbálásra támaszkodik, hanem több kulcsfolyamat reprodukálható módon is ellenőrizhető.
...
### 5.2.1 Alkalmazásbelépés és hitelesítési állapotkezelés
...
Az hitelesítési kontextus feladatai:
...
A token a böngésző `localStorage` területén kerül tárolásra. Ez egyszerű és jól használható megoldás a jelenlegi projektméret mellett, mivel a rendszer elsődleges célja nem vállalati szintű munkamenet-kezelés, hanem stabil helyi és bemutatási használat biztosítása.
...
- útvonalparamétereket biztosít az oldalak számára,
...
A későbbi hibajavítások során különösen hasznosnak bizonyult az a döntés, hogy a router már kliensoldalon kiszűri a nyilvánvalóan hibás azonosítókat. Ez hozzájárul ahhoz, hogy a felhasználó ne tudjon például a `/tournaments/new` útvonalról tévesen olyan útvonalakra navigálni, amelyek már létező versenyazonosítót várnak.
...
A áttekintő oldal a felhasználó versenyeinek áttekintését adja, a profiloldal a felhasználói adatok és jelszó kezelésére szolgál. A versenylétrehozó oldal a kezdeti konfigurációs folyamat belépési pontja, míg a versenyáttekintő oldal az adott verseny összefoglaló nézete.
...
- globális hitelesítési állapot kontextuson keresztül,
- oldalszintű `useState` alapú lokális állapot,
- `useEffect` alapú adatbetöltés,
- útvonalparaméterekből származtatott kontextus.

Ez a stratégia tudatos egyszerűsítés. A rendszer jelenlegi mérete és a képernyők felelősségi köre mellett a lokális állapotkezelés jól átlátható marad. Az hitelesítési állapot kivételével a legtöbb adat csak egy-egy konkrét oldal számára releváns, ezért nem indokolt teljes alkalmazásszintű globális tároló használata.
...
5. a hitelesítési kontextus frissül, az alkalmazás privát nézetre vált.
...
5. a kapcsolódó csoportállás nézet újratöltve már az új állást mutatja.
...
- az útvonal- és lekérdezési paraméterek helyességét,
...
A frontend oldalon a központi API-réteg a backendtől érkező hibákat egy fordító modulon keresztül magyar nyelvű, konzisztens üzenetekké alakítja. Ez különösen fontos olyan műveleteknél, mint a mérkőzésindítás, az eredményrögzítés vagy a lezárt versenyek állapotkezelése.

A backend oldalon a útvonalak több helyen explicit védelmet tartalmaznak:
...
A fejlesztés során különösen fontossá vált, hogy egy hibás kliensoldali navigáció vagy egy rossz paraméterű kérés ne okozzon szerveroldali összeomlást. Ennek megfelelően a későbbi javítások során megerősítésre került a útvonalparaméterek validációja és az invalid azonosítók kezelése.
...
- az útvonalkezelő az ownership szolgáltatás vagy közvetlen lekérdezés segítségével ellenőrzi a tulajdonjogot,
...
A megvalósítás részeként létrejött egy olyan bemutató adatkészlet-generálás mechanizmus is, amely lehetővé teszi egy előre ismert tesztfelhasználó és több feltöltött verseny automatikus létrehozását. Ez nem csupán kényelmi funkció, hanem a rendszer bemutathatóságának fontos eleme.

Ennek köszönhetően:
- a fejlesztés reprodukálható adatkészlettel végezhető,
- a fő nézetek és üzleti folyamatok azonnal kipróbálhatók,
- a konzulensi vagy bizottsági bemutató előtt nem szükséges manuálisan felépíteni a bemutató állapotot.
...
A rendszer megvalósítása során a tervezett architektúra lényegében érvényesült. A backendben a útvonalkezelő–szolgáltatási–modell felosztás világosan elkülöníti a kéréskezelést, az üzleti logikát és a perzisztenciát. A frontend oldalon a saját routerre, hitelesítési kontextusra és oldalanként elkülönített nézetekre épülő szerkezet támogatja az adminisztratív használatot.

A megvalósítás erőssége, hogy a rendszer nem pusztán adatrekordok kezelésére alkalmas, hanem a verseny lebonyolításának több kulcsfontosságú lépését is ténylegesen automatizálja. Ide tartozik a sorsolás, az állásszámítás, a továbbjutás, a egyenes kieséses és az ütemezés támogatása. Ezt egészíti ki a naplózás, az export és a bemutató adatkészlet-generálás, amelyek a rendszer gyakorlati használhatóságát tovább erősítik.

**Javasolt ábrahelyek:**  
*5.1. ábra – A backend útvonalkezelő–szolgáltatási–modell szerkezete*  
*5.2. ábra – A frontend fő nézetei és navigációs kapcsolataik*  
*5.3. ábra – Példa adatáramlás: eredményrögzítés és csoportállás frissítés*

# 6. Tesztelés és validáció

A rendszer fejlesztése során a tesztelés célja nem pusztán az volt, hogy az egyes képernyők megjelennek-e, hanem az is, hogy a versenykezeléshez kapcsolódó üzleti logikák helyesen és reprodukálható módon működnek-e. Mivel a projekt több, egymásra épülő funkcionális területből áll – például hitelesítés, sorsolás, állásszámítás, egyenes kieséses ág generálása, ütemezés, export és audit –, ezért a validáció több szinten történt.

A tesztelés során három fő megközelítés jelent meg:
- automatizált logikai és füstteszt jellegű ellenőrzések,
...
- az egyenes kieséses ágat felépítik,
...
- körmérkőzéses párosítási logika,
- csoportállás és holtverseny-feloldás,
- egyenes kieséses párosítási logika,
...
- eredményrögzítés,
- csoportállás lekérdezés,
- egyenes kieséses felépítése,
- globális ütemezés generálása.

A füsttesztek célja annak ellenőrzése volt, hogy a rendszer fő működési útvonalai nem törtek el egy-egy módosítás után.
...
- egyenes kieséses szakasz ellenőrzése,
- export és nyilvános kijelzőnézet használata.
...
### 6.1.3 Bemutató adatkészlet-generálás mint tesztelési eszköz

A tesztelési stratégia részeként létrejött egy bemutató adatkészlet-generálás mechanizmus is, amely ismert tesztfelhasználóval és előre feltöltött, eltérő állapotú versenyekkel tölti fel az adatbázist. Ez a megoldás két okból is hasznos:

- reprodukálható kiindulóállapotot biztosít a manuális és félautomatikus tesztekhez;
- lehetővé teszi, hogy a rendszer bemutatási célra gyorsan újraépíthető legyen.

A bemutató adatkészlet különösen jól használható volt olyan nézetek tesztelésére, ahol egyszerre kellett kész, futó és még függő állapotú objektumokat ellenőrizni.
...
- A csoportállás valóban a szabályok szerinti sorrendet adja-e?
- A egyenes kieséses a megfelelő játékosokkal épül-e fel?
...
### TC-06 Check-in és sorsoláslezárás
...
2. A sorsolás lezárása.  
3. A generált csoportok és mérkőzések lekérdezése.  
...
### TC-07 Körmérkőzéses párosítás helyessége
...
- Csonka körmérkőzéses rendszer esetén a terhelés közel egyenletes.  
...
### TC-10 Csoportállás számítás és holtverseny-feloldás
...
**Tesztlépések:**  
1. A csoportállás végpont meghívása.  
2. A visszakapott sorrend összevetése az elvárt holtverseny-feloldási szabály logikával.  
...
### TC-11 Egyenes kieséses ág generálása

**Kapcsolódó követelmények:** FR-10, AC-07, AC-08

**Kiinduló állapot:**  
A csoportkör csoportállás adatai rendelkezésre állnak.

**Tesztlépések:**  
1. A továbbjutók kiválasztása.  
2. A egyenes kieséses ág generálása.  
3. A generált mérkőzések ellenőrzése.  

**Elvárt eredmény:**  
- A megfelelő számú továbbjutó kerül kiválasztásra.  
- A rendszer kiemelésalapú párosításokat hoz létre.  
- A egyenes kieséses szakasz megjeleníthető és nyomon követhető.  

### TC-12 Csak egyenes kieséses kategória kezelése

**Kapcsolódó követelmények:** FR-03, FR-10, AC-08

**Kiinduló állapot:**  
Csak egyenes kieséses formátumú kategória létezik.
...
**Elvárt eredmény:**  
- A rendszer csoportkör nélkül is képes teljes egyenes kieséses működésre.  
...
### TC-14 Globális ütemezés több kategóriára
...
**Tesztlépések:**  
1. A globális ütemezési végpont meghívása.  
...
### TC-15 Nyilvános kijelzőnézet és végpont
...
**Tesztlépések:**  
1. A nyilvános kijelző végpont vagy nyilvános kijelző oldal megnyitása.  
2. A megjelenített mérkőzések ellenőrzése.  
...
**Kiinduló állapot:**  
A versenyhez léteznek játékos-, mérkőzés- és csoportállás adatok.
...
### TC-20 Bemutató adatkészlet-generálás és bemutathatóság

**Kapcsolódó követelmények:** AC-11

**Kiinduló állapot:**  
Üres vagy törölt bemutató adatbázis.

**Tesztlépések:**  
1. A bemutató adatkészlet-generálás futtatása.  
2. Bejelentkezés az ismert tesztfelhasználóval.  
3. A fő adminisztratív oldalak végigellenőrzése.  

**Elvárt eredmény:**  
- A bemutató felhasználó és a bemutató versenyek létrejönnek.  
- A rendszer bemutatási környezetben azonnal használható.  

### TC-21 Pályaütközés tiltása mérkőzésindításkor
...
### TC-22 Játékosütközés tiltása mérkőzésindításkor
...
**Tesztlépések:**  
1. Megpróbálni egy lezárt verseny mérkőzésének eredményét módosítani.  
...
## 6.3 Speciális esetek és kritikus helyzetek
...
Páratlan számú játékos esetén a klasszikus körmérkőzéses rendszer generálás virtuális BYE pozíciót igényel. Az ellenőrzés során biztosítani kellett, hogy:
...
### 6.3.2 Csonka körmérkőzéses rendszer nagyobb mezőnynél

Nagyobb játékosszám esetén a teljes körmérkőzés túl sok mérkőzést eredményezne. Emiatt szükséges volt ellenőrizni:
- minden játékos a konfigurált számú mérkőzést kapja-e meg,
...
A csoportállás logika tesztelésének egyik legnehezebb része a holtversenyek kezelése volt. Külön vizsgálni kellett:
- két játékos egymás elleni eredmény alapján eldönthető holtversenyét,
- több játékos körbeverés jellegű holtversenyét,
- olyan helyzetet, ahol a mini-tábla után is szükség van további holtverseny-feloldási feltételekre,
...
- a csoportállás újraszámítása megtörténik-e,
...
### 6.3.5 Visszalépés és játék nélküli győzelem
...
- a még le nem játszott mérkőzések megfelelően kezelődjenek,
- a csoportállás ne torzuljon indokolatlanul,
- a rendszer a választott policy szerint működjön.

Különösen jelentős speciális eset, amikor a visszalépés már részben lejátszott csoportkörben történik, és a rendszernek el kell döntenie, hogy a korábbi eredmények megmaradnak-e.
...
- a kliensoldali útvonal-validációt,
...
A mérkőzések életciklusának egyik kritikus speciális esete, amikor a szervező vagy a felület hibás sorrendben próbál mérkőzést indítani. A jelenlegi rendszerben külön vizsgálni kellett, hogy:
- ugyanazon a pályán egyszerre ne futhasson két mérkőzés,
- ugyanaz a játékos ne szerepelhessen párhuzamosan két futó mérkőzésben,
...
Ez az speciális eset azért fontos, mert a játékvezetői rotáció nem lehet erősebb kényszer, mint maga a mérkőzésütemezés. Ha nincs elegendő játékvezető, a rendszernek kontrolláltan kell viselkednie, nem pedig hibás állapotot létrehoznia.

### 6.3.9 Lezárt verseny utólagos korrekciója

Külön speciális eset-ként jelent meg a lezárt versenyek eredményeinek javítása. Teljes tiltás esetén a rendszer túl merev, korlátlan szerkeszthetőség esetén viszont sérül a lezárt állapot jelentése. A jelenlegi megoldás egy köztes modellt alkalmaz: a verseny lezárása után az eredmények zároltak, de adminisztratív feloldással ideiglenesen javíthatók.
...
### 6.3.11 Bemutató adatkészlet és bemutatási állapot

A rendszer bemutatási használatára tekintettel külön speciális eset-nek tekinthető, hogy az adatbázis üres vagy nem megfelelő állapotban van. Ennek megoldására szolgál a bemutató adatkészlet-generálás, amely:
...
Ez nem klasszikus üzleti speciális eset, de a projekt értékelhetősége és bemutathatósága szempontjából mégis kulcsfontosságú.

## 6.4 A tesztelés eredményeinek összegzése

A végrehajtott tesztek alapján megállapítható, hogy a rendszer fő funkcionális követelményei teljesülnek. A backend képes kezelni a versenyek, kategóriák, játékosok és mérkőzések teljes alapvető életciklusát, valamint a csoportkörös és egyenes kieséses alapú lebonyolítás fő logikáit is.

Különösen erős területek:
- a körmérkőzéses rendszer és csoportállás logika,
- az egyenes kieséses ág generálása,
...
- a bemutató adatkészletre épülő bemutathatóság.
...
Összességében a validáció eredménye azt mutatja, hogy a rendszer szakdolgozati keretek között egy működőképes, reprodukálhatóan tesztelhető és bemutatható prototípusnak tekinthető, amely a fő követelmények döntő részét teljesíti.

**Javasolt ábrahelyek:**  
*6.1. ábra – Tesztelési stratégia: logikai, integrációs és manuális szintek*  
*6.2. ábra – Példa tesztfolyamat: check-in → sorsoláslezárás → csoportállás → egyenes kieséses szakasz*  
...
- részleges körmérkőzéses rendszer lebonyolítás támogatására,
- mérkőzéseredmények szettekre bontott rögzítésére,
- csoportállás számításra és holtversenyek feloldására,
- továbbjutók meghatározására,
- egyenes kieséses ág generálására és követésére,
- mérkőzések ütemezésére,
- játékvezetők automatikus rotációjára,
- hátralévő mérkőzések becsült kezdési idejének frissítésére,
- nyilvános kijelzőnézet biztosítására,
...
- a teljes és csonka körmérkőzéses rendszer párosítások,
- a badminton szabályok szerinti eredményvalidáció,
- a csoportállás és holtverseny-feloldási logika,
- a továbbjutási és egyenes kieséses mechanizmus,
...
Szintén fontos eredmény a reprodukálható bemutató adatkészlet-generálás kialakítása. Ez fejlesztési, tesztelési és bemutatási szempontból egyaránt előnyös, mivel lehetővé teszi, hogy a rendszer ismert belépési adatokkal és előre előállított, eltérő állapotú versenyekkel azonnal kipróbálható legyen.

A rendszer jelenlegi állapotában alkalmas arra, hogy szakdolgozati bemutató keretében bemutassa a fő adminisztratív folyamatokat, és szemléltesse a tollaslabda versenykezeléshez kapcsolódó legfontosabb üzleti logikákat.
...
További erősség a moduláris szerkezet. A backendben a útvonalkezelő–szolgáltatási–modell rétegződés, a frontendben pedig a nézetek és közös komponensek elkülönítése javítja a karbantarthatóságot. Ez a szerkezet lehetővé teszi, hogy új funkciók vagy módosítások később célzottan, a teljes rendszer átírása nélkül kerüljenek be.
...
Korlátként jelenik meg az is, hogy a rendszer elsődlegesen helyi vagy fejlesztői környezetben futtatott alkalmazásként készült. Bár a hosztolás technikailag megoldható lenne, ez nem képezte a dolgozat elsődleges fókuszát. Emiatt a jelenlegi változat inkább prototípus- és bemutatási használatra optimalizált.

A lebonyolítási logikák terén is vannak határok. A körmérkőzéses rendszer, csoportállás és egyenes kieséses támogatás erős, ugyanakkor a rendszer nem kezel minden lehetséges speciális versenyformát. Nem célja például:
...
- vizuálisan fejlettebb ágrajznézet,
- jobban strukturált áttekintő oldal,
- gyorsabb adminisztratív műveleti útvonalak.

A versenylogika oldalán további lehetőséget jelentene:
- összetettebb kiemelési rendszer,
...
- egyszerű hosztolt bemutató verzió,
- automatikus bemutató adatok automatikus visszaállítása,
- publikus megosztható nyilvános kijelzőlinkek,
...
- reprodukálható tesztelés és bemutató állapot kialakításában.
...
A rendszer legfontosabb szakmai eredménye, hogy a domainben lényeges automatizmusokat – csoportkör-generálás, csoportállás számítás, holtverseny-kezelés, egyenes kieséses felépítés és ütemezés – egységes alkalmazásba integrálja. Bár a megoldás jelenlegi formájában még nem tekinthető teljes körű, végleges terméknek, szakdolgozati keretek között jól használható, érdemi eredményt képviselő és továbbfejleszthető alkalmazásként értelmezhető.
