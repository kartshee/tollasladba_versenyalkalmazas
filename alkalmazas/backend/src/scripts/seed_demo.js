...
// Rögzített szetteredménnyel zár le egy meccset — a holtverseny-demóhoz szükséges,
// ahol a pontos eredmények számítanak a tiebreak feloldásakor.
function createCustomFinish(baseMatch, winnerId, sets) {
    return {
        ...baseMatch,
        status: 'finished',
        resultType: 'played',
...
async function ensureDemoUser({ replace = false }) {
    ...
    if (replace && user) {
        console.log('  → Meglévő demo adatok törlése...');
...
// Holtverseny-feloldás demókategória
// ---------------------------------------------------------------------------
// A standings service egyik legösszetettebb funkciója: ha több játékosnak azonos
// a győzelmek száma, a rendszer először mini-tabellát számol (csak az érintett
// játékosok egymás elleni meccseiből), majd ha az is egyenlő, az összesített
// szett- és pontkülönbséghez nyúl.
//
// Ebben a kategóriában szándékosan ilyen helyzet jön létre:
//
//   Vezér Dávid: 3 győzelem → egyértelműen 1. hely
//   Holt Ádám, Holt Bence, Holt Csaba: mindenki 1-1 győzelem → holtverseny
//
//   Mini-tabella (A, B, C egymás elleni meccsek):
//     A beat B, C beat A, B beat C → mindenki 1-1 győzelem a mini-tabellában
//     Szettarány mindenkinél 0 (2-2), de pontarány: A=+8, B=-4, C=-4
//     → A egyértelműen 2. hely
//
//   B és C még mindig holtversenyben → összesített pontarány dönti el:
//     B összesített ptDiff=-17, C összesített ptDiff=-23
//     → B 3. hely, C 4. hely
//
// A Tabella oldalon ez a feloldási folyamat vizuálisan megjelenik.
async function seedTiebreakDemoCategory({ tournament, refereeNames }) {
    const category = await Category.create({
        tournamentId: tournament._id,
        name: 'Holtverseny-feloldás demó',
        gender: 'mixed',
        ageGroup: 'Nyílt',
        format: 'group',
        groupsCount: 1,
        ...
        unresolvedTiePolicy: 'shared_place',
        status: 'in_progress',
        drawLockedAt: minutesFrom(new Date(), -240),
    });

    const players = await createPlayers({
        tournament,
        category,
        names: ['Holt Ádám', 'Holt Bence', 'Holt Csaba', 'Vezér Dávid'],
        checkedInCount: 4,
        clubA: 'Demo Klub A',
        clubB: 'Demo Klub B',
    });
    ...
    // Segédfüggvény az alap meccs-dokumentum felépítéséhez
    function buildBase(p1, p2, court, minuteOffset) {
        const startAt = minutesFrom(new Date(), minuteOffset - 240);
        const endAt = minutesFrom(startAt, 35);
        return {
            tournamentId: tournament._id,
            categoryId: category._id,
            ...
            player1: p1._id,
            player2: p2._id,
            pairKey: makePairKey(p1._id, p2._id),
            round: 'group',
            roundNumber: court,
            drawVersion: 1,
            resultType: 'played',
            ...
            startAt,
            endAt,
            umpireName: refereeNames[court % refereeNames.length] ?? '',
            sets: [],
            winner: null,
        };
    }

    // 6 mérkőzés — szándékosan meghatározott eredményekkel a tiebreak-forgatókönyvhöz.
    // A sets mezőben p1 az első játékos (player1), p2 a második (player2) pontszáma.
    //
    // Eredmények:
    //   D beats A: 21-18, 21-19  → D: setDiff+2, ptDiff+5;   A: -2, -5
    //   D beats B: 21-15, 21-14  → D: setDiff+2, ptDiff+13;  B: -2, -13
    //   D beats C: 21-11, 21-12  → D: setDiff+2, ptDiff+19;  C: -2, -19
    //   A beats B: 21-15, 21-14  → A: setDiff+2, ptDiff+13;  B: -2, -13
    //   C beats A: 21-18, 21-19  → C: setDiff+2, ptDiff+5;   A: -2, -5
    //   B beats C: 21-16, 21-17  → B: setDiff+2, ptDiff+9;   C: -2, -9
    //
    // Összesített: D(3gy), A(1gy, ptDiff+3), B(1gy, ptDiff-17), C(1gy, ptDiff-23)
    const matchDefs = [
        { p1: pD, p2: pA, winner: pD, sets: [{ p1: 21, p2: 18 }, { p1: 21, p2: 19 }], court: 1, offset: 0 },
        { p1: pD, p2: pB, winner: pD, sets: [{ p1: 21, p2: 15 }, { p1: 21, p2: 14 }], court: 2, offset: 0 },
        { p1: pD, p2: pC, winner: pD, sets: [{ p1: 21, p2: 11 }, { p1: 21, p2: 12 }], court: 1, offset: 45 },
        { p1: pA, p2: pB, winner: pA, sets: [{ p1: 21, p2: 15 }, { p1: 21, p2: 14 }], court: 2, offset: 45 },
        { p1: pC, p2: pA, winner: pC, sets: [{ p1: 21, p2: 18 }, { p1: 21, p2: 19 }], court: 1, offset: 90 },
        { p1: pB, p2: pC, winner: pB, sets: [{ p1: 21, p2: 16 }, { p1: 21, p2: 17 }], court: 2, offset: 90 },
    ];

    await Match.insertMany(
        matchDefs.map(({ p1, p2, winner, sets, court, offset }) =>
            createCustomFinish(buildBase(p1, p2, court, offset), winner._id, sets)
        )
    );
...
async function seedConflictTestCategory({ tournament, refereeNames }) {
    ...
    await Match.insertMany([
        // Fut: players[0] vs players[1] a 2-es pályán
        {
            tournamentId: tournament._id,
            categoryId: category._id,
            groupId: group._id,
        ...
        // Ütemezve a 2-es pályára (ütközés a futó mecccsel)
        {
            tournamentId: tournament._id,
            categoryId: category._id,
            groupId: group._id,
        ...
        // players[0] már fut egy meccset, és itt is szerepel (játékos-ütközés)
        {
            tournamentId: tournament._id,
            categoryId: category._id,
            groupId: group._id,
        ...
        // Ütemezetlen meccs (nincs pálya és időpont) — ezzel tesztelhető az indítási validáció
        {
            tournamentId: tournament._id,
            categoryId: category._id,
            groupId: group._id,
        ...
        // Egy eredménnyel már rögzített meccset utólag érvényteleníteni csak admin művelettel lehet.
        {
            ...
            player1: players[1]._id,
            player2: players[2]._id,
            pairKey: makePairKey(players[1]._id, players[2]._id),
            round: 'group',
            status: 'finished',
            roundNumber: 6,
            drawVersion: 1,
            resultType: 'played',
            voided: true,
            voidReason: 'Adminisztrátori döntés: a meccs technikai okokból érvénytelen.',
            courtNumber: 3,
            startAt: minutesFrom(now, -60),
            endAt: minutesFrom(now, -25),
            actualStartAt: minutesFrom(now, -60),
            actualEndAt: minutesFrom(now, -25),
            resultUpdatedAt: minutesFrom(now, -25),
            umpireName: refereeNames[0] ?? '',
            sets: [{ p1: 21, p2: 14 }, { p1: 18, p2: 21 }],
            winner: null
        }
    ]);
...
async function seedCompletedGroupPlayoffCategory({ tournament, refereeNames }) {
    ...
    await Match.insertMany([
        ...
        createWoFinish(bronzeBase, semiLosers[0])
    ]);
...
// ---------------------------------------------------------------------------
// VERSENY-SZINTŰ SEEDERS
// ---------------------------------------------------------------------------

async function seedDraftTournament(owner) {
    console.log('\n[1/3] Tervezet verseny generálása...');
    ...
    console.log('  → Előkészítés alatt álló kategória...');
    const setup = await seedSetupCategory({ tournament });

    console.log('  → Check-in nyitva kategória...');
    const checkin = await seedCheckinOpenCategory({ tournament });

    console.log('  → Sorsolás lezárva kategória...');
...
async function seedRunningTournament(owner) {
    ...
    console.log('  → Férfi egyéni amatőr (group+playoff, futó döntő)...');
    const c1 = await seedRunningGroupPlayoffCategory({ tournament, refereeNames });

    console.log('  → Női egyéni amatőr (vegyes meccs állapotok: lejátszott, RET, várakozó)...');
    const c2 = await seedRunningGroupCategory({ tournament, refereeNames });

    console.log('  → Hibavédelmi tesztkategória (pálya- és játékos-ütközések, érvénytelenített meccs)...');
    const c3 = await seedConflictTestCategory({ tournament, refereeNames });

    console.log('  → Meghívásos playoff (playoff-only formátum)...');
    const c4 = await seedPlayoffOnlyCategory({ tournament, refereeNames });

    console.log('  → Holtverseny-feloldás demó (3 játékos azonos győzelemszámmal)...');
    const c5 = await seedTiebreakDemoCategory({ tournament, refereeNames });
    ...
    await createAuditLogs({
        ...
        items: [
            ...
            },
            {
                ...
                summary: 'Előkészítve: pályaütközés, játékosütközés, ütemezetlen indítás és érvénytelenített meccs tesztelésére.'
            },
            {
                entityType: 'category',
            ...
            {
                entityType: 'category',
                entityId: String(c5._id),
                categoryId: c5._id,
                action: 'match.result_recorded',
                summary: `Holtverseny-demó meccsek rögzítve: ${c5.name}`
            }
        ]
    });
...
async function seedFinishedTournament(owner) {
    ...
    console.log('  → Férfi egyéni lezárt főverseny (group+playoff, WO bronzban)...');
    const c1 = await seedCompletedGroupPlayoffCategory({ tournament, refereeNames });

    console.log('  → Senior női egyéni (group-only, WO utolsó meccsen)...');
...
async function printSummary(tournaments) {
    console.log('\n══════════════════════════════════════════');
    console.log('  Demo seed sikeresen elkészült.');
    console.log('══════════════════════════════════════════\n');
    ...
    for (const tournament of tournaments) {
        const counts = {
            kategóriák: await Category.countDocuments({ tournamentId: tournament._id }),
            játékosok: await Player.countDocuments({ tournamentId: tournament._id }),
            nevezések: await Entry.countDocuments({ tournamentId: tournament._id }),
            csoportok: await Group.countDocuments({ tournamentId: tournament._id }),
            meccsek: await Match.countDocuments({ tournamentId: tournament._id }),
            fizetési_csoportok: await PaymentGroup.countDocuments({ tournamentId: tournament._id }),
            napló: await AuditLog.countDocuments({ tournamentId: tournament._id })
        };

        console.log(`\n  ${tournament.name}`);
        console.log(`    státusz:      ${tournament.status}`);
        console.log(`    tournamentId: ${String(tournament._id)}`);
        console.log('    tartalom:', counts);
    }

    console.log('\n══════════════════════════════════════════');
    console.log('  Ajánlott tesztelési útvonalak');
    console.log('══════════════════════════════════════════\n');
    console.log('1. Tervezet verseny / Női egyéni – check-in nyitva');
    console.log('   → Csoportos befizetés egyben rendezése (Szegedi Női Tollas Kör).\n');
    console.log('2. Aktív verseny / Hibavédelmi tesztkategória');
    console.log('   → Futó meccs a 2-es pályán. Tesztelhető:');
    console.log('      - másik meccs indítása ugyanarra a pályára (pályaütközés)');
    console.log('      - másik meccs indítása ugyanazzal a játékossal (játékos-ütközés)');
    console.log('      - ütemezetlen meccs indítása (nincs pálya/időpont)');
    console.log('      - érvénytelenített meccs megjelenítése a meccslistán.\n');
    console.log('3. Aktív verseny / Holtverseny-feloldás demó');
    console.log('   → Tabella oldal: Holt Ádám, Holt Bence, Holt Csaba mindegyike 1 győzelmes.');
    console.log('      A rendszer mini-tabellával, majd összesített pontarányával oldja fel.\n');
    console.log('4. Aktív verseny / Férfi egyéni amatőr');
    console.log('   → Group+playoff, futó döntő az 1-es pályán, nyitott fizetési csoport.\n');
    console.log('5. Lezárt verseny');
    console.log('   → Végeredmény, WO/RET példák, zárol lezárt eredményjavítás.\n');
}
...
async function main() {
    if (!process.env.MONGO_URI) {
        throw new Error('Hiányzó MONGO_URI környezeti változó. Ellenőrizd a .env fájlt.');
    }

    const replace = process.argv.includes('--replace');

    if (replace) {
        console.log('⚠  --replace flag aktív: a meglévő demo adatok törlésre kerülnek.');
    }

    console.log('Adatbázis kapcsolat felépítése...');
    await mongoose.connect(process.env.MONGO_URI);
    console.log('Kapcsolat OK.\n');
